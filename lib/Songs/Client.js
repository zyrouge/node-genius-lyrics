...
class SongsClient {
     ...
     */
    constructor(key, config = { requestOptions: {baseURL: this.key ? CONSTANTS.BASE_URL : CONSTANTS.UN_BASE_URL} }) {
    ...
    search(q) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let result = [];
                if (this.key) {
                    const res = await axios.get(`/search?q=${term}`, {
                    ...
                    const res = await axios.get(`/search/multi?per_page=5&q=${term}`, this.config.requestOptions);
