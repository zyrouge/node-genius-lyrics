        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if(this.key) {
                    ...
                    const res = await axios.get(`${CONSTANTS.UN_BASE_URL}/search/multi?per_page=5&q=${term}`);
                ...
                const tracks = result.filter(s => s.type === "song").map(s => new Song(s.result, this.key, true));
                resolve(tracks);
            } catch(e) {
    ...
    get(q) {
        ...
        return new Promise((resolve, reject) => {
            ...
            .then(res => {
                ...
                const track = new Song(res.data.response.song, this.key, false);
                resolve(track);
            })
            .catch(e => {
                if(e && e.response && e.response.status && e.response.status == 401) reject(CONSTANTS.INV_TOKEN);
